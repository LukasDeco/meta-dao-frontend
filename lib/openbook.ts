...
export const isPass = (order: OrderBookOrder, proposal?: ProposalAccountWithKey) =>
  proposal?.account.openbookPassMarket.equals(order.market)!!;

export const isBid = (order: OrderBookOrder) => {
  const isBidSide = order.side === "bids";
  if (isBidSide) {
    return true;
  }
  return false;
...
// FIXME: make this work we may need to fetch this...
export const isPartiallyFilled = (order: OrderBookOrder): boolean => {
...
export const isEmptyOrder = (order: OrderBookOrder): boolean =>
  order.size === 0;

export const isClosableOrder = (order: OrderBookOrder): boolean =>
  new BN(order.size).eq(BN_0);

//TODO evaluate this crazy change
export const isOpenOrder = (order: OrderBookOrder, markets: Markets): boolean => {
...
export const isCompletedOrder = (order: OrderBookOrder, markets: Markets): boolean => {
  ...
  return isEmptyOrder(order) && !isOpen;
};

export const isBidOrAsk = (order: OrderBookOrder) => {
  const isBidSide = order.side === "bids";
  if (isBidSide) {
    return true;
  }
...
// TODO: delete because looks like we don't use this?
// export const totalInOrder = (orders: OrderBookOrder[]) => {
//   let sumOrders = [];
...
export const totalUsdcInOrder = (orders: OrderBookOrder[]) => {
  let sumOrders = [];
  sumOrders = orders.map((order) => order.size * order.price * 10_000);

  const totalValueLocked = sumOrders.reduce((partialSum, amount) => partialSum + amount, 0);
  return numeral(totalValueLocked).format(NUMERAL_FORMAT);
};

export const totalMetaInOrder = (orders: OrderBookOrder[]) => {
  let sumOrders = [];
  sumOrders = orders.map((order) => order.size);
